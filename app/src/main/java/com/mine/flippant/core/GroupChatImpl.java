...
public class GroupChatImpl extends QBMessageListenerImpl<QBGroupChat> implements Chat, QBMessageSentListener<QBGroupChat> {
    ...
    public GroupChatImpl(GameActivity gameActivity) {
        Log.d(TAG, "GroupChatImpl");
        this.gameActivity = gameActivity;
    }

    public void joinGroupChat(QBDialog dialog, QBEntityCallback callback){
        initManagerIfNeed();
        Log.d(TAG, "joinGroupChat: " + dialog.getName());
        if(groupChat == null) {
            Log.d(TAG, "joinGroupChat: groupChat == null, " + dialog.getRoomJid());
    ...
    private void join(final QBGroupChat groupChat, final QBEntityCallback callback) {
        ...
        Toast.makeText(gameActivity, "Joining room...", Toast.LENGTH_LONG).show();
            ...
            public void onSuccess(final Void result, final Bundle bundle) {
                ...
                gameActivity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(gameActivity, "Join successful", Toast.LENGTH_LONG).show();
                    }
                });
    ...
    public void sendMessage(QBChatMessage message) throws XMPPException, SmackException.NotConnectedException {
        Log.v(TAG, "sendMessage: " + message.getBody());
        if (groupChat != null) {
            try {
                groupChat.sendMessage(message);
                Log.v("GroupChatSendMessage", message.getBody());
            } catch (SmackException.NotConnectedException nce){
                nce.printStackTrace();
                Toast.makeText(gameActivity, "Can't send a message, You are not connected to chat", Toast.LENGTH_SHORT).show();
            } catch (IllegalStateException e){
                e.printStackTrace();
                Toast.makeText(gameActivity, "You are still joining a group chat, please wait a bit", Toast.LENGTH_LONG).show();
            }
        } else {
            Toast.makeText(gameActivity, "Join unsuccessful", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void processMessage(QBGroupChat groupChat, QBChatMessage chatMessage) {
        ...
        gameActivity.showMessage(chatMessage);
    }
